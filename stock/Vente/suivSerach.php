...
if (isset($_POST['ajouter'])) {
    ...
    if (!empty($idprod) && !empty($idf) && !empty($qte) && !empty($prix) && !empty($total)) {
        ...
        $query = "INSERT INTO entree (`idfour`,`idprod`,`prixAchat`,`QTE`,`total`,`status`,`NmFact`) 
Values ('$idf','$idprod','$prix','$qte','$total','$status','$factr')";
...
Values ('$des','$code','$Img','$cat','$quantite','$unite','$PrixU','$PrixV','  $mag')";
        ...
        if ($result) {
            echo "<script> alert('Data saved');</script>";
            header('location: suiviAchat.php');
        } else {
            echo "<script> alert('Ooops!');</script>";
        }
    } else {
...
<body>
    ...
    function Achat()
    {
        ...
        $query = $connexion->query("SELECT  * FROM produit,entree where  produit.id=entree.idprod and idfour='$idf'");
    ...
    <div class="list-prod">
        ...
        <div class="row" style="background-color:#fff ;height:80px;position:relative;bottom:100px;right:60px;">
            ...
            <div class=" col-auto" style="position: relative;bottom:20px;left:110px;">
                ...
                <input type="password" class="form-control" id="inputPassword2" style="background-color: #eee; border-radius:none;" placeholder="<?php echo Achat(); ?> DH  ">
    ...
    <div class="search" style="position: relative; bottom:-90px; left:1050px;z-index:100;">
        ...
        <form method="post">
            <div class="inf" style="position: relative;bottom:70px;">
                ...
                <div class="col-auto">
                    ...
                    <input type="number" class="form-control" id="inputPassword2" name="qt" style="position:relative;left:850px;width: 180px;bottom:78px;">
                </div>
                <div class="col-auto">
                    <p style="position: relative;left:1050px;bottom:150px;">Prix Achat</p>
                    <input type="number" class="form-control" id="inputPassword2" name="achat" style="position:relative;left:1050px;width: 180px;bottom:155px;">
                </div>
                <a href="" style="position:relative;left:1230px;top:15px;">
        ...
        <div class="bg" style="position:relative ;bottom:250px;">
            ...
            <div class="list-prod">
                <table class="content-tab" style="width:75% ;position:relative;right:530px;bottom:30px;">
                    ...
                    $factr = $_GET['numFact'];
                    $connexion = mysqli_connect('localhost', 'root', '', 'Stock') or die(mysqli_error($connexion));
                    $query = $connexion->query("SELECT * FROM `entree`,`facture` ,`produit` where 
                produit.id=entree.idprod and entree.NmFact=facture.NumFacture and entree.NmFact= '$factr' and entree.idfour='$numfact' ");
                    while ($row = $query->fetch_array()) {
                    ...
                    ?>
                        ...
                        <tbody>
                            ...
                            <tr data-href="LigneAchat.php?ent=<?php echo $row['idEnt']; ?>">
                                <a href="#">
                                    ...
                                    <td>
                                        ...
                                        <?php if ($row['status'] == 'Valider') : ?>
                                            <a href="" class="btnn" style="background-color:#1DBF57;"> </i><?php echo $row['status'] ?> </a>

                                        <?php else : ?>
                                            <a href="" class="btnn" style="background-color:rgb(153, 0, 0) ;"> </i><?php echo $row['status'] ?> </a>
                                    </td>
